...
@section('section')
    <div class="main-panel">
        <div class="content-wrapper">
            <div class="row">
                <a href="{{route('file.raqdro',['testuuid'=>$testuuid,'user_uuid'=>$user_uuid,'datauser'=>$datauser,'file'=>1])}}">
                    <span class="mdi mdi-file-excel mdi-24px"></span> შედეგები ცხრილის სახით</a>
                {{--                @php--}}
                ...
                @foreach($diagrama as $i => $diagramaitem)
                    ...
                    @if($targamot == $datauser)
                        @if(!empty($diagramaitem->id))
                            <a href="{{route('raqdro.pasuxfile',['testuuid'=>$testuuid,'diagramid'=>$diagramaitem->id])}}">დიაგრამა {{$i + 1}}</a>
                        @endif
                    @endif
                @endforeach
            </div>
