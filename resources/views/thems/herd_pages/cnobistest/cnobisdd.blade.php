...
@section('section')
    ...
    <div class="col-12 grid-margin">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">ხანმოკლე მეხსიერების ტესტირება. ცნობის ტესტი.</h4>
                <form class="form-sample" method="post" action="{{route('cnobistest.test.add')}}">
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                ...
                @if(isset($swori[0]) && !empty($swori[0]))
                    <div class="row">
                        <div class="table-sorter-wrapper col-lg-12 table-responsive">
                            <form method="post" action="{{route('cnobistest.testpart.add')}}">
                                @csrf
                                ...
                                <table id="sortable-table-2" class="table table-striped">
                                    ...
                                    <tr>
                                        <th>#</th>
                                        <th class="sortStyle">დასამახსოვრებელი კომბინაცია<i class="mdi mdi-chevron-down"></i></th>
                                        <th class="sortStyle">გამოსაცნობი რიცხვი<i class="mdi mdi-chevron-down"></i></th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($swori as $i => $sworiitem)
                                        <tr>
                                            <td>{{$i+1}}</td>
                                            <td><input value="{{$sworiitem['gamosacnobi']}}" class="form-control" name="gamosacnobi[]">
                                            </td>
                                            <td><input value="{{$sworiitem['dasamaxsovrebeli']}}" class="form-control"
                                                       name="dasamaxsovrebeli[]"></td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
